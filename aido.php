...
class AiToolExecutor
{
    ...
    private function executeCommand(string $command): string
    {
        ...
        $output = shell_exec($command . ' 2>&1'); // 2>&1 also catches error output
	...
	// load recent 10 history etries for context
        $messages = array_merge($messages, array_slice($this->history, -10));

	// 3. tool call loop (max. 5 iterations for avoiding endless loops)
        for ($i = 0; $i < 5; $i++) {
            ...
            // important: add assistant answer to history (no matter if text or function call)
            $messages[] = $message;
	    ...
	    // check for function call
            if (!empty($message['function_call'])) {
                ...
                if ($functionName === 'run_command' && isset($arguments['command'])) {
                    ...
                    // Das Ergebnis des Tools mit der speziellen Rolle 'function' hinzufügen
                    $messages[] = [
                        'role' => 'function',
                        'name' => 'run_command',
                        'content' => $result
            ...
            // when being here we got a final message by the assistant 
